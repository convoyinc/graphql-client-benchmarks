...
export default class UpdateHundredTwentyFiveObservers extends Benchmark {
  numObservers = 125;

  static metadata = {
    title: `update (125 observers)`,
  };
  ...
  constructor(client: Client, example: Example) {
    ...
    for (let i = 0; i < this.numObservers; i++) {
      this.updatedResponses.push(overlayResponse(example.partials[i % this.example.partials.length].response, update));
    }
  }
